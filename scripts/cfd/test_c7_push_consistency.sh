...
wait_mark_stage_with_retrigger() {
  ...
  for i in $(seq 1 "${retries}"); do
    ...
    if (( i % retrigger_every == 0 )); then
      curl -sS -X POST "${INDEX_BASE_URL}/api/v1/index-price/calculate?symbol=${SYMBOL}" >/dev/null || true
      curl -sS -X POST "${MARK_BASE_URL}/api/v1/mark-price/calculate?symbol=${SYMBOL}" >/dev/null || true
    fi
...
if ! wait_log_match "${POSITION_LOG}" "${pos_start_line}" "${trade_pos_pattern}" 80 0.2; then
...
if ! wait_log_match "${PRIVATE_PUSH_LOG}" "${push_start_line}" "${trade_push_pattern}" 80 0.2; then
...
mark_stage_hits="$(wait_mark_stage_with_retrigger \
  "${pos_mark_start_line}" \
  "${push_mark_start_line}" \
  "${mark_pos_pattern}" \
  "${mark_push_pattern}" \
  "${MARK_WAIT_RETRIES}" \
  "${MARK_WAIT_INTERVAL}" \
  "${MARK_RETRIGGER_EVERY}" || true)"
